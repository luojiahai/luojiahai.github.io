...
1. **Agent loop engine**: the brain, where all decisions originate
1. **Tool system**: 40+ built-in tools plus MCP extensions
1. **Memory system**: solving the “AI forgets everything” problem
1. **Context compression**: keeping token costs under control
1. **Permission and security layer**: at the bottom, holding everything accountable
...
1. **Microcompact**: offload large tool execution results to a cache
1. **Context Collapse**: summarize intermediate conversation, keeping only key information
1. **Autocompact**: full summary compression when context usage crosses a threshold
1. **Reactive Compact**: emergency fallback, triggered when the API returns a 413 error
...
Claude Code has a `--dangerously-skip-permissions` flag (YOLO mode) that bypasses all permission prompts. It’s not as unguarded as it sounds. A shadow AI runs quietly in the background, classifying every action the main model wants to take.
