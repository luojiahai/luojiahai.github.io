---
layout: home
title: luojiahai

hero:
  name: luojiahai
  text: <code>你好，世界！</code><span class="spinner"></span>
---

<Terminal>

```text
$ whoami
```

<<<./data/whoami.json {json}

</Terminal>

你好，我是 **luojiahai**。我是广州土著，目前住在墨尔本，正在努力工作挣钱、养家糊口。**luojiahai** 是我名字**罗嘉海**的拼音，我在网上都用这个网名。我名字的粤拼是 **law ka hoi**。我还用英文名 **geoffrey**。

我是敲代码的，专业做全栈软件开发和一些站点可靠性工程。现就职于 [rea group](https://www.rea-group.com/)（asx:rea），旗下运营 [realestate.com.au](https://realestate.com.au/)，澳洲最大的房地产网站。这是我的[简历](/zh/resume)。在敲代码之外，我喜欢吃饭、做饭和逛超市。我玩《微软飞行模拟》，主要飞空客。这是我[使用的硬件和软件](/zh/use)。

在这些地方找到我：

<LinkButton color="default" icon="github" href="https://github.com/luojiahai">github</LinkButton>
<LinkButton color="default" icon="x" href="https://x.com/luojiahai">x</LinkButton>
<LinkButton color="magenta" icon="instagram" href="https://instagram.com/luojiahai">instagram</LinkButton>
<LinkButton color="light-blue" icon="linkedin" href="https://linkedin.com/in/luojiahai">linkedin</LinkButton>
<LinkButton color="pink" icon="bilibili" href="https://space.bilibili.com/866961">哔哩哔哩</LinkButton>
<LinkButton color="yellow" icon="sinaweibo" href="https://weibo.com/ljiahai">微博</LinkButton>

或者发邮件给我：<EmailAddress local="luo" domain="jiahai.co" />

---

这个网站使用 [vitepress](https://vitepress.dev/) 静态生成，部署在 [github pages](https://docs.github.com/en/pages) 上。
